...
const { fetchKlines, VALID_TIMEFRAMES } = require('./exchange');
  ...
  sendMenu(bot, chatId, msgId, text, {
    ...
    reply_markup: {
      inline_keyboard: [
        ...
        [{ text: '➕ Add Pair', callback_data: 'st_managepair' }],
...
async function showStStatus(bot, chatId) {
  try {
    ...
    let pairText = Object.entries(pairs).map(([t, tfs]) => `  \u2022 ${t}: ${tfs.join(', ')}`).join('\n') || '  \u2014';
    bot.sendMessage(chatId, `\ud83d\udcc8 <b>ST Simulasi Status</b>\n\nRunning: ${running ? '\u2705 Yes' : '\u274c No'}\nPairs:\n${pairText}\n\nOpen trades: ${openCount}\nClosed trades: ${closedCount}`, { parse_mode: 'HTML' });
...
function showStManagePair(bot, chatId) {
  ...
  if (keys.length) {
    ...
    rows.push([{ text: '\u2795 Add New', callback_data: 'st_managepair' }]);
  ...
  register(bot) {
      ...
      try {
        if (session.cmd === 'st_managepair') {
          ...
          if (session.step === 'timeframes') {
            ...
            const insPair = db.prepare('INSERT OR IGNORE INTO st_pairs (ticker, timeframe) VALUES (?, ?)');
            db.transaction(() => {
    ...
    function handleCallback(query) {
      ...
      try {
        ...
        if (data.startsWith('st_managepair_add_')) {
          const ticker = data.replace('st_managepair_add_', '');
          conv[chatId] = { cmd: 'st_managepair', step: 'timeframes', data: { ticker } };
          bot.sendMessage(chatId, `Ticker: ${ticker}\nMasukkan timeframe (pisahkan koma). Contoh: <code>15m,1h,4h</code>`, { parse_mode: 'HTML' });
          return { action: null };
        }
